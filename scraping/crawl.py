...
class CrawlPrepare:
    ...
    def get_requested_crawl(self):
        crawl_list=args.choose_crawl
        if args.choose_crawl and 'all' in args.choose_crawl:
            print("force crawl triggered")
            crawl_list=self.all_job_listings

        if args.choose_crawl and 'all' not in args.choose_crawl:
            print("Checking if job listing match")
            for i in args.choose_crawl:
                if i not in self.all_job_listings:
                    print(i, "is not is current job listing")
                    args.choose_crawl.remove(i)

        if not crawl_list:
            print("No crawlable links available")
    ...
    def determine_runnable_listings(self):
        ...
        # print(non_crawlable_listings)
        crawlable_listings = self.all_job_listings - non_crawlable_listings

        return crawlable_listings

# print(args.force_crawl)
if args.force_crawl:
    print(CrawlPrepare().get_forced_crawl())
if args.choose_crawl:
    print(CrawlPrepare().get_requested_crawl())
